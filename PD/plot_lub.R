# source("~/PD/wallen1.R")
# source("~/PD/wallen2.R")
# rej4 is the result from ~/PD/wallen1.R while rej5 is the result from ~/PD/wallen2.R
...
###### Wallen2
CDIlist2 = readRDS("~/Downloads/Lub6.rds")
...
p11 = ggplot(dataset, aes(x = group, y = value, color = group))  + theme_bw()+  
  ...
  theme(legend.position = "none",
        ...
        axis.ticks.x = element_blank(),axis.text.y = element_text(
          angle = 90,          # 刻度标签旋转90度（竖直显示）
          hjust = 0.5,         # 水平居中
          vjust = 0.5        ))+   # 移除 x 轴刻度线
  stat_summary(fun = "mean", geom = "point", color = "red", size = 3, shape = 18) +
  # geom_jitter(position = position_jitter(0.4), alpha = 0.2, size=1.5)+  # 添加均值红点
...
p21 = ggplot(dataset, aes(x = group, y = value, color = group)) + theme_bw()+  
  ...
  theme(legend.position = "none",
        panel.grid = element_blank(),
        axis.title.x = element_blank(),  # 移除 x 轴标题
        axis.ticks.x = element_blank(),  # 移除 x 轴刻度标签
...
###### Wallen2
CDIlist2 = readRDS("~/Downloads/Lub12.rds")
otutable=CDIlist2$otu
meta = CDIlist2$meta
meta$Age=as.numeric(meta$Age)
keepsample = which(meta$Age >= age_cut & meta$Age < age_cut2)
