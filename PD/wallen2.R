...
# linda
lib_cut=1
res <- LinDA::linda(Y, ZZ, paste('~',formula),type = "count", alpha=0.05,lib.cut = lib_cut)
rej.ld <- which(res$output[[1]]$reject)

# poda
rej=PODA( Y = Y, u = u, confun = confun,rep_time=500,tau = 1,fdrnomial=0.05 ,lib_cut=0, 
          pre_cut=0,nCore=9)
rej=rej$rej

# locom
tt = LOCOM::locom(otu.table = t(Y), Y = data.frame(factor(u)), C =confun, fdr.nominal = 0.05,seed=100, n.cores = 10,filter.thresh = 0)
...
rej5=match(rownames(Y)[rej],taxs)
