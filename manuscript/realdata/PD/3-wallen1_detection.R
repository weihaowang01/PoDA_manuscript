...
library(dplyr)
library(tidytree)
source("~/Desktop/newPODA/manuscript/helper_function.R")
source("~/Desktop/newPODA/manuscript/PODA.R")
source("~/Desktop/newPODA/manuscript/imputation.R")
##################################################################################################
##################################################################################################
##################################################################################################
pre_cut=0.1
...
Z=cbind(as.matrix(u),as.matrix(z1),as.matrix(z2))
ZZ=data.frame(factor(u),factor(z1),factor(z2))
confoun=as.matrix(cbind(as.matrix(z1),as.matrix(z2)))
colnames(confoun)=c("z1","z2")
colnames(Z)=c("u","z1","z2")
colnames(ZZ)=c("u","z1","z2")
formula <- 'u+z1+z2'
formulaa <- '~u+z1+z2'
n=ncol(Y)
m=nrow(Y)
