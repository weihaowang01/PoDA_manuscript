...
A <- readRDS("~/manuscript/simulation/Results/main/A7unifSE.rds")
simlist <- t(A[1:3,])
colind <- c(1,2,5,3)
...
simlist <- simlist*100
rownames(simlist) <- c("power.IPOD","power.LinDA","power.locom","power.ANCOMBC2",
                       "fdr.IPOD","fdr.LinDA","fdr.locom","fdr.ANCOMBC2")
...
f <- function(i) {
  index <- i
  data.frame(
    class = rep(c("Power(%)","Empirical FDR(%)"), each=4),
    method = rep(c('PoDA','LinDA','LOCOM','ANCOM-BC2'), 2),
    value = simlist[, index],
    se = error[, index] 
  )
}
...
ptest1 <- ggplot(out, aes(x=sig.prob, y=value, fill=method)) + 
  theme_bw() +
  geom_bar(
    data = out[out$q=="5",],
    stat="identity",
  ...
  scale_fill_manual(values = c('red','#8074C8','#00A08799','#E64B3599')) +
...
ptest3 <- ggplot(out, aes(x=sig.prob, y=value, fill=method)) + 
  theme_bw() +
  geom_bar(
    data = out[out$q=="5",],
    stat="identity",
...
ggsave("~/manuscript/simulation/Plots/main/A7unif.png", p.comb, width = 4*3.5, height =2.5*3.5, limitsize = FALSE ,bg = "white")
