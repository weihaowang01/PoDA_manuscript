...
# 假设你的数据加载和处理步骤保持不变
A <- readRDS("~/weihaowang/PoDAsuppsim/simulation/SE/unifse.rds")
simlist <- t(A[1:3,])
colind <- c(1,2,5,3)
...
ptest1 <- ggplot(out, aes(x=sig.prob, y=value, fill=method)) +  # 主数据集使用全部数据
  ...
  # 再绘制q=="0.5"的数据，通过alpha区分而不是重新设置填充
  geom_bar(
    data = out[out$q=="0.5",],
    stat="identity",
    position=position_dodge(0.7),
...
ggsave("~/weihaowang/PoDAsuppsim/errorbar画图/A7unif.png", p.comb, width = 4*3.5, height =2.5*3.5, limitsize = FALSE ,bg = "white")
