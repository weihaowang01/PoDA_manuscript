...
################### LinDA ###################
#############################################
pseu_poda_power=cbind(pseu[,2],imp[,2],LN[,2],Repl[,2])
pseu_poda_fdr=cbind(pseu[,7],imp[,7],LN[,7],Repl[,7])
pseu_poda=cbind(pseu_poda_power,pseu_poda_fdr)

# #############################################
# ################### ACBC ####################
# #############################################
# pseu_poda_power=cbind(pseu[,4],imp[,4],LN[,4],Repl[,4])
# pseu_poda_fdr=cbind(pseu[,9],imp[,9],LN[,9],Repl[,9])
# pseu_poda=cbind(pseu_poda_power,pseu_poda_fdr)
...
# ggsave("~/manuscript/simulation/Plots/zero/A2new_ld.png", p.comb, width = 4*3.5, height =2.5*3.5, limitsize = FALSE ,bg = "white")
