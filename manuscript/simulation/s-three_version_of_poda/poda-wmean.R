...
PODAFUNnew = function(y_poda,omega.i,standarad_sigma1,lmd){
  ...
  mean.alpha.est<-sum(test_x_1$gamma/standarad_sigma1-omega.i/standarad_sigma1**2  )/(sum(1/(standarad_sigma1)))
  ...
  mean.alpha.est<-sum(test_x_1.new$gamma/standarad_sigma1-omega.i/standarad_sigma1**2  )/(sum(1/(standarad_sigma1)))
  if(is.na(mean.alpha.est)){
    return(c("fail"))
  }
  y_poda.new<-y_poda+mean.alpha.est/(standarad_sigma1)
...
PODAwmean = function( Y = Y, u = u, confoun = NULL ,rep_time=50,tau = 1,fdrnomial=0.05,fdrmethod="BH",lib_cut=1000,
                      pre_cut=0.2,nCore=9){
  ##################################################################
  ####################### preparation ##############################
  ##################################################################
