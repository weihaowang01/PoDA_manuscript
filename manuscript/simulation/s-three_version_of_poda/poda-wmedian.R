...
PODAFUNnew = function(y_poda,omega.i,standarad_sigma1,lmd){
  ...
  mean.alpha.est=-weightedMedian(omega.i-test_x_1$gamma*standarad_sigma1, w = 1/standarad_sigma1,ties="min")
  ...
  mean.alpha.est=-weightedMedian(omega.i-test_x_1.new$gamma*standarad_sigma1, w = 1/standarad_sigma1,ties="min")
  if(is.na(mean.alpha.est)){
    return(c("fail"))
  }
  y_poda.new<-y_poda+mean.alpha.est/(standarad_sigma1)
...
PODAwmedian = function( Y = Y, u = u, confoun = NULL ,rep_time=50,tau = 1,fdrnomial=0.05,fdrmethod="BH",lib_cut=1000,
                 pre_cut=0.2,nCore=9){
  ##################################################################
  ####################### preparation ##############################
  ##################################################################
